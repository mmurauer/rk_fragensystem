 ...
 */
function question_category_form($contexts, $pageurl, $current, $recurse=1, $showhidden=false, $showquestiontext=false,$myquestions=false) {
    global $CFG, $OUTPUT;

    $cnew = new stdClass();
    $cnew->id=1;
    $cnew->contextlevel=10;
    $cnew->instanceid=0;
    $cnew->path='/1';
    $cnew->depth=1;
...
/// Get all the existing categories now
    $catmenu = rk_question_category_options($contexts, false, 0, true);
	
    $strcategory = get_string('category', 'quiz');
    $strshow = get_string('show', 'quiz');
    $streditcats = get_string('editcategories', 'quiz');

	$select = new single_select($pageurl, 'category', $catmenu, $current, null, 'catmenu');
 	$select->set_label(get_string('selectacategory', 'question'));
 	echo $OUTPUT->render($select);
 	echo "</div>\n";
 	
	//if($myquestions)
		//popup_form ('myquestions.php?'.$pageurl->get_query_string().'&amp;category=', $catmenu, 'catmenu', $current, '', '', '', false, 'self', "<strong>$strcategory</strong>");
	//else
		//popup_form ('myquestions.php.php?'.$pageurl->get_query_string().'&amp;category=', $catmenu, 'catmenu', $current, '', '', '', false, 'self', "<strong>$strcategory</strong>");

   /* echo '<form method="get" action="myquestions.php" id="displayoptions">';
 ...
 */
function rk_question_category_options($contexts, $top = false, $currentcat = 0, $popupform = false, $nochildrenof = -1) {
    global $CFG, $USER, $DB;
    $pcontexts = array();
        ...
        foreach ($categories as $category) {
            if ($category->contextid == $pcontext){
                ...
                if ($currentcat!= $cid || $currentcat==0) {
                    $countstring = " (".$DB->count_records('question',array('category'=>$category->id,'createdby'=>$USER->id)).")";
...
function question_list($contexts, $pageurl, $categoryandcontext, $cm = null,
    ...
    $qtypemenu = array("multichoice" => "Multiple-Choice","truefalse"=>"Wahr/Falsch","shortanswer"=>"Kurzantwort");
    ...
    echo '<form method="post" action="myquestions.php">';
    ...
    foreach ($questions as $question) {
        ...
        if (question_has_capability_on($question, 'edit', $question->category)) {
            // hide-feature
            if($question->hidden) {
                echo "<a title=\"$strrestore\" href=\"myquestions.php?".$pageurl->get_query_string()."&amp;unhide=$question->id&amp;sesskey=$USER->sesskey\"><img
                        src=\"$CFG->pixpath/t/restore.gif\" alt=\"$strrestore\" /></a>";
            } else {
                echo "<a title=\"$strdelete\" href=\"myquestions.php?".$pageurl->get_query_string()."&amp;deleteselected=$question->id&amp;q$question->id=1\"><img
 ...
 */
function question_showbank($tabname, $contexts, $pageurl, $cm, $page, $perpage, $cat, $recurse, $showhidden, $showquestiontext, $myquestions = false){
    ...
    $OUTPUT->box_start('generalbox questionbank');
    $OUTPUT->heading(get_string('questionbank', 'question'));
    // $OUTPUT->heading(get_string('questionbank', 'question'), '', 2);

    question_category_form($contexts->having_one_edit_tab_cap($tabname), $pageurl, $cat, $recurse, $showhidden, $showquestiontext, $myquestions);

    // continues with list of questions
    display_question_list($contexts->having_one_edit_tab_cap($tabname), $pageurl, $cat, isset($cm) ? $cm : null,
...
/*function question_edit_setup($edittab, $requirecmid = false, $requirecourseid = true){
    ...
    if ($thiscontext){
        $contexts = new question_edit_contexts_rk($thiscontext);
...
*/class question_edit_contexts_rk{
    var $allcontexts;
    /**
     * @param current context
     */
    function question_edit_contexts_rk($thiscontext){
