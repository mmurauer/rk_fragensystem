 ...
 */
    ...
    $questionids = optional_param('questionids', '', PARAM_INT);
	...
	if(!block_rk_fragesystem_check_owner($cm->instance))
    ...
    if (!$quiz->questions and has_capability('mod/quiz:manage', $context)) {
        redirect($CFG->wwwroot . '/blocks/rk_fragesystem/edit.php?cmid=' . $cm->id . '&courseid='.$course->id);
    }
...
/// Get number for the next or unfinished attempt
	$sql = ('SELECT MAX(attempt)+1 FROM {quiz_attempts} WHERE quiz = '.$quiz->id.' AND 
			userid = '.$USER->id.' AND timefinish > 0');

    if(!$attemptnumber = (int)$DB->get_field_sql($sql)) {
        $attemptnumber = 1;
    }
...
/// Check number of attempts
    $numberofpreviousattempts = $DB->count_records_select('quiz_attempts', "quiz = '{$quiz->id}' AND " .
...
/// Check password access
    ...
    if (!empty($quiz->password) and empty($SESSION->passwordcheckedquizzes[$quiz->id])) {
        ...
        if (optional_param('cancelpassword', false, PARAM_BOOL)) {
            ...
            redirect($CFG->wwwroot . '/blocks/rk_fragesystem/view.php?q=' . $quiz->id);
            ...
            $url = $CFG->wwwroot . '/blocks/rk_fragesystem/attempt.php?q=' . $quiz->id;
...
/// Load attempt or create a new attempt if there is no unfinished one
    ...
    if (!$attempt) {
        ...
        if ($oldattempts = $DB->get_records_select('quiz_attempts', "quiz = '$quiz->id'
...
/// Load all the questions and states needed by this script
    ...
    $pagelist = quiz_questions_on_page($quiz->questions, $page);
    ...
    if ($newattempt and $attempt->attempt > 1 and $quiz->attemptonlast and !$attempt->preview) {
        ...
        if (!$lastattemptid = $DB->get_field('quiz_attempts', 'uniqueid', 'quiz', $attempt->quiz, 'userid', $attempt->userid, 'attempt', $attempt->attempt-1)) {
...
/// Process form data /////////////////////////////////////////////////
    ...
    if ($responses = data_submitted() and empty($responses->quizpassword)) {
        ...
        $event = (array_key_exists('markall', $responses)) ? QUESTION_EVENTSUBMIT :
         ($finishattempt ? QUESTION_EVENTSAVE : QUESTION_EVENTSAVE);
        ...
        if (!$success) {
            ...
            print_error('errorprocessingresponses', 'question',
                    $CFG->wwwroot . '/blocks/rk_fragesystem/attempt.php?q=' . $quiz->id . $pagebit);
        }
...
/// Finish attempt if requested
    if ($finishattempt) {
        ...
        if (!$closequestions = $DB->get_records_sql($sql)) {
...
/// Update the quiz attempt and the overall grade for the quiz
    ...
    if ($finishattempt) {
		...
		redirect($CFG->wwwroot . '/blocks/rk_fragesystem/review.php?attempt='.$attempt->id.'&courseid='.$courseid.'&id='.$q, 0);
...
/// Print the quiz page ////////////////////////////////////////////////////////
    ...
    if ($ispreviewing) {
        ...
        //print_single_button($CFG->wwwroot.'/blocks/rk_fragesystem/attempt.php', $buttonoptions, get_string('startagain', 'quiz'));
        echo '</div>';
    ...
    // Start the form
    $quiz->thispageurl = $CFG->wwwroot . '/blocks/rk_fragesystem/attempt.php?q=' . s($quiz->id) . '&amp;page=' . s($page).'&courseid='.$courseid;
    $quiz->cmid = $cm->id;
